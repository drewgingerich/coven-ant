...
public abstract class AbstractSerialThread
{
    ...
    private bool guessPortName = false;
                                ...
                                bool guessPortName = false)
    {
        ...
        this.guessPortName = guessPortName;
	...
	{			
		...
		foreach (string thisDevice in devices) {				
			if (thisDevice.StartsWith("/dev/tty.usb") || thisDevice.StartsWith("/dev/ttyUSB")) {
				selectedDevice = thisDevice;
				Debug.Log("Guessing that arduino is device: " + selectedDevice);
				break;
			}
		}		
